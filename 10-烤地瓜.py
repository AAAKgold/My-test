...
class SweetPotato:
    ...
    def cook(self,cookedTime):
        ...
        #因为这个方法被调用了多次，为了能够在一次调用这个方法的时候，能够获取到上一次调用这个方法时的cookedTime
        #所以需要在此把cookedTime保存到这个对象的属性中，因为属性不会随着方法的调用而结束（一个方法被调用的时候）
        #是可以用局部变量来保存数据的，但是这个方法定义结束之后这个方法中的所有数据就没有了
    ...
    def addCondiments(self,item):
        #因为item这个变量指向了一个佐料，所以 接下来需要将item放到append里面
